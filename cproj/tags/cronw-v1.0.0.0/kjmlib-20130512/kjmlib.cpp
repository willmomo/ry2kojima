...
// 分を取得
//-----------------------------------------------------------------------------
int kjm::dateTime::get_Minute() const {
	SYSTEMTIME st;
...
// 時を取得
//-----------------------------------------------------------------------------
int kjm::dateTime::get_Hour() const {
	SYSTEMTIME st;
	FileTimeToSystemTime(&this->m_ft, &st);
	return st.wHour;
}
...
// 日を取得
//-----------------------------------------------------------------------------
int kjm::dateTime::get_Day() const {
	SYSTEMTIME st;
	FileTimeToSystemTime(&this->m_ft, &st);
	return st.wDay;
}
...
// 月を取得
//-----------------------------------------------------------------------------
int kjm::dateTime::get_Month() const {
	SYSTEMTIME st;
	FileTimeToSystemTime(&this->m_ft, &st);
	return st.wMonth;
}
...
// 曜日を取得
//-----------------------------------------------------------------------------
int kjm::dateTime::get_Week() const {
	SYSTEMTIME st;
	FileTimeToSystemTime(&this->m_ft, &st);
...
void kjm::log::shift(const _tstring& fname) {
	if ( kjm::path::fileExists( (fname + _T(".9")) ) ) {
		DeleteFile( ( fname + _T(".9") ).c_str() );
	}

	for (int i = 8; i >= 1; i--) {
		MoveFile(
			kjm::util::sprintf_str(_T("%s.%d"), fname.c_str(), i).c_str(),
			kjm::util::sprintf_str(_T("%s.%d"), fname.c_str(), i + 1).c_str());
	}

	MoveFile(fname.c_str(), (fname + _T(".1")).c_str());
}
...
void kjm::log::write(LPCTSTR format, ...) {
	...
	{	// ログシフトのための処理
		if (this->m_maxLogSize > 0) {
			struct _stat buffer;
			if (_tstat(fname.c_str(), &buffer) == 0) {
				if (buffer.st_size > this->m_maxLogSize) {
					this->shift(fname);
				}
			}
		}
