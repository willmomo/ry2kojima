...
Function WriteIniString(strSection, strKey, strValue, strIniFile)
	...
	If blnWritten = False Then
		If UCase(strCurSection) <> UCase(strSection) Then
...
'----------------------------------------------------------------------
' sntpsv のインストール状況を調べる
'----------------------------------------------------------------------
Function IsSntpsvInstalled()
	Dim logLevel
	logLevel = LogEnter("IsSntpsvInstalled")
	
	Dim strSql
	Dim objList
	
	strSql = "select * from Win32_Service where Name='SntpService'"
	Set objList = GetObject("winmgmts:").ExecQuery(strSql)
	If objList.Count = 0 Then
		IsSntpsvInstalled = False
	Else
		IsSntpsvInstalled = True
	End If
	
	Call LogLeave("IsSntpsvInstalled", IsSntpsvInstalled, logLevel)
End Function
...
Function StopSntpsv()
	...
	For Each objService In objList
		...
		LogWrite "nRet = " & nRet
		If nRet = 5 Then StopSntpsv = True	'
		Exit For
	Next
	
	Call LogLeave("StopSntpsv", StopSntpsv, logLevel)
End Function
...
Function StopSntpw()
	...
	WriteIniString "debug", "command", "exit", "C:\tools\sntpw\sntpw.ini"
...
Function CopySntpdIni()
	...
	Dim objFso
	Set objFso = WScript.CreateObject("Scripting.FileSystemObject")
	objFso.CopyFile "C:\tools\sntpw\sntpw.ini", "C:\tools\sntpw\sntpsv.ini"
...
Function CopySntpd()
	...
	Dim objFso
	Set objFso = WScript.CreateObject("Scripting.FileSystemObject")
	objFso.CopyFile "sntpsv.exe", "C:\tools\sntpw\sntpsv.exe"
	
	Const vbNormalFocus = 1			'通常のウィンドウ、かつ最前面のウィンドウにします
	Dim objShell
	Set objShell = WScript.CreateObject("WScript.Shell")
	objShell.Run "C:\tools\sntpw\sntpsv.exe install", vbNormalFocus, True
...
Function DoMain()
	...
	Else
		...
		'3 sntpw.ini を sntpsv.ini にコピーする
		CopySntpdIni
	End If
	
	'4 sntpsv をコピーする(コピー先は C:\tools\sntpw)
	CopySntpd
