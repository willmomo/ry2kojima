...
typedef NTSTATUS (WINAPI *NTQSI)(_SYSTEM_INFORMATION_CLASS, PVOID, ULONG, PULONG);

NTSTATUS NtQuerySystemInformation(_SYSTEM_INFORMATION_CLASS sic, PVOID buf, ULONG bufsize, PULONG size) {
	NTSTATUS ret = STATUS_SUCCESS;
	HMODULE hLibModule = LoadLibrary(_T("ntdll.dll"));
	if (hLibModule) {
		NTQSI func = (NTQSI)GetProcAddress(hLibModule, "NtQuerySystemInformation");
		if (func) {
			ret = (func)(sic, buf, bufsize, size);
		}
