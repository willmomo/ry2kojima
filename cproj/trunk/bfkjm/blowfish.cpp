...
static long hl(long n) {
	return ((n & 0xff000000) >> 24) | ((n & 0x00ff0000) >> 8) | ((n & 0x0000ff00) << 8) | ((n & 0x000000ff) << 24);
}

std::vector<char> kjm::blowfish::encrypt(const std::string& k, const std::string& s) {
	BLOWFISH_CTX ctx;
	Blowfish_Init(&ctx, (unsigned char *)k.c_str(), k.length());

	// PKCS#5 Padding ÇÃèàóù
	std::vector<char> work(s.begin(), s.end());
	int pad = 8 - (work.size() % 8);
	for (int i = 0; i < pad; i++) {
		work.push_back(pad);
	}

	char* p = work.data();
	for (size_t i = 0; i < work.size(); i += 8, p += 8) {
		unsigned long* pxl = (unsigned long*)(p + 0);
		unsigned long* pxr = pxl + 1;
		*pxl = htonl(*pxl);
		*pxr = htonl(*pxr);
		Blowfish_Encrypt(&ctx, pxl, pxr);
		*pxl = ntohl(*pxl);
		*pxr = ntohl(*pxr);
	}
	return work;
...
std::vector<char> kjm::blowfish::decrypt(const std::string& k, const std::string& s) {
	BLOWFISH_CTX ctx;
	Blowfish_Init(&ctx, (unsigned char *)k.c_str(), k.length());
	...
	for (size_t i = 0; i < work.size(); i += 8, p += 8) {
		...
		*pxl = htonl(*pxl);
		*pxr = htonl(*pxr);
		Blowfish_Decrypt(&ctx, pxl, pxr);
		*pxl = ntohl(*pxl);
		*pxr = ntohl(*pxr);
	}

	// PKCS#5 Padding ÇÃèàóù
	for (size_t i = work.back(); i > 0; --i) {
		work.pop_back();
	}
