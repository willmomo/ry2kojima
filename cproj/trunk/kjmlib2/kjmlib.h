//---------------------------------------------------------------------
// kjmlib : Version 0.2015.9.16
//
// 2015/06/18	kjm::thread を使いまわし時に安全に動くように修正。
// 2015/05/06	kjm::odbcUtilを使うときはUSE_ODBC32を定義するように変更。
// 2015/05/01	log::get/set_RotateCount, get/set_RotateSize 関数を追加。
// 2015/02/23	mp3関連の文字列を扱う関数を追加
// 2014/11/07	vs2010から：back_inserterを使うためにインクルードが必要
// 2014/11/05	fileInfo, folderInfo を拡張
// 2014/07/18	io.h, fcntl.h をインクルードするように修正。
// 2014/07/18	fileInfo クラスの代入演算子、代入が不十分なのを修正。
//
...
namespace kjm {
	...
	public:
		...
		static std::basic_string<TCHAR> get_module_file_name(HMODULE hModule = NULL) {
			std::vector<TCHAR> filename(MAX_PATH);
			if (::GetModuleFileName(hModule, &filename[0], filename.size()) == FALSE) {
...
#define FT_19700101_SEC		(FT_19700101 / FT_1SEC)
	...
	public:
		...
		// プロセス生成
		//-------------------------------------------------------------
		BOOL Create(const std::basic_string<TCHAR>& commandLine) {
