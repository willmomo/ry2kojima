...
void install(LPCTSTR path) {
	...
	if (hSCManager == NULL) {
		...
		_tcout << _T("install error: OpenSCManager error: ") << dwErr << _T(" \n");
		switch (dwErr) {
			case ERROR_ACCESS_DENIED:
				_tcout << _T("要求されたアクセスは拒絶されました。") << _T("\n");
				break;
			case ERROR_DATABASE_DOES_NOT_EXIST:
				_tcout << _T("指定されたデータベースは存在しません。") << _T("\n");
				break;
			case ERROR_INVALID_PARAMETER:
				_tcout << _T("指定されたパラメータは無効です。") << _T("\n");
				break;
			default:
				_tcout << _T("未知のエラーです。") << _T("\n");
				break;
		}
		return;
	}
