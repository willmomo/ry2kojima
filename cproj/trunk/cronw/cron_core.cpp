...
static void doAction(const cronEntry& entry) {
	kjm::GetLog().write(_T("$ action: %s(%d): s_popt access !!"), __FILE__, __LINE__);
...
unsigned __stdcall crond_thread(void *parg) {
	kjm::GetLog().write(_T("日本語 cron thread: start"));
	...
	kjm::GetLog().write(_T("$ action: cron_core.cpp(%d): loop start !!"), __LINE__);

	int prevMin = -1;	// 前回実行した分(分が変化した時点で実行する)
	while (s_fLoop) {
		//kjm::GetLog().write(_T("$ action: cron_core.cpp(%d): loop { !!"), __LINE__);
		Sleep(1000);

		kjm::GetLog().write(_T("$ action: cron_core.cpp(%d): get now !!"), __LINE__);
		kjm::dateTime now = kjm::dateTime::now();
		kjm::GetLog().write(_T("$ action: cron_core.cpp(%d): got now !!"), __LINE__);

		if (prevMin != now.get_Minute()) {
			prevMin = now.get_Minute();

			kjm::GetLog().write(_T("$ action: cron_core.cpp(%d): actionEntries before !!"), __LINE__);
			actionEntries(now, cronEntries);
			kjm::GetLog().write(_T("$ action: cron_core.cpp(%d): actionEntries after !!"), __LINE__);
		}
		//kjm::GetLog().write(_T("$ action: cron_core.cpp(%d): loop } !!"), __LINE__);
	}
	return 0;
