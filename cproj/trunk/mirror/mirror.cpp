...
//---------------------------------------------------------------------
bool BackupMirror::copy_file(const kjm::fileInfo& fiOrg, const kjm::_tstring& strMir, const kjm::_tstring& orgBase) {
	kjm::tickCounter tc;
	kjm::_tstring strMode;

	// シンボリックリンクの時は、速やかに終了
	if (fiOrg.isSymbolicLink()) {
		g_log.writee(_T("'%s' はシンボリックリンクでした"), fiOrg.fullName().c_str());
		return true;
	}
	...
	if (!get_TestMode()) {
		...
		if (ret == FALSE) {
			...
			kjm::_tstring temp = kjm::util::chomp(kjm::util::formatMessageBySystem(dwErr));
...
// する。
//---------------------------------------------------------------------
bool BackupMirror::is_exclude(const kjm::_tstring& strFname) {
	kjm::_tstring lowerFname = kjm::util::toLower(strFname);

	for (std::vector<kjm::_tstring>::size_type i = 0; i < m_excludeList.size(); i++) {
		if (lowerFname.find(m_excludeList[i]) != kjm::_tstring::npos) {
			if (this->get_Verbose()) {
...
// バックアップ
//---------------------------------------------------------------------
bool BackupMirror::backup(const kjm::_tstring& strOrg, const kjm::_tstring& strMir) {
	static kjm::_tstring s_orgBase;
	bool bRet = true;
...
// ミラーリング
//---------------------------------------------------------------------
bool BackupMirror::mirror(const kjm::_tstring& strOrg, const kjm::_tstring& strMir) {
	bool bRet = true;
...
//---------------------------------------------------------------------
void load_exclude_list(const kjm::_tstring& strFname, std::vector<kjm::_tstring>& excludeList) {
	_tout << _T("除外定義読込み: '" << strFname << "'.\n");

	kjm::_tstring dst = kjm::environment::expandEnvironmentVariables(strFname);
...
int _tmain(int argc, TCHAR** argv) {
	...
	std::vector<kjm::_tstring> opts = cl.getOptList();

	BackupMirror bakMir;
	
	std::vector<kjm::_tstring> excludeList;

	for (std::vector<kjm::_tstring>::size_type i = 0; i < opts.size(); i++) {
		if (opts[i] == _T("mode")) {
			kjm::_tstring optArg = cl.getOptArg(opts[i]);
	...
	for (std::vector<kjm::_tstring>::size_type j = 0; j < excludeList.size(); j++) {
...
theEnd:
	...
	if (g_log.get_Stop()) {
		_tout << _T("ログファイルは出力されていません(--logpathで出力先を指定してください)。\n");
	} else {
