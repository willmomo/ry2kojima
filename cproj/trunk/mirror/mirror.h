...
private:
	// ファイルのコピー処理
	bool copy_file(const kjm::fileInfo& fiOrg, const kjm::_tstring& strMir, const kjm::_tstring& orgBase);
	...
	// バックアップ処理
	bool backup(const kjm::_tstring& strOrg, const kjm::_tstring& strMir);

	// ミラーリング処理
	bool mirror(const kjm::_tstring& strOrg, const kjm::_tstring& strMir);
	
	// 除外判定を行う
	bool is_exclude(const kjm::_tstring& strFname);

public:
	// 原本パスの取得/設定
	kjm::_tstring get_OriginalPath() const { return m_strOrg; }
	void set_OriginalPath(const kjm::_tstring& newValue) { m_strOrg = newValue; }

	// 複製パスの取得/設定
	kjm::_tstring get_MirrorPath() const { return m_strMir; }
	void set_MirrorPath(const kjm::_tstring& newValue) { m_strMir = newValue; }
	...
	// 除外ファイル一覧の取得/設定
	std::vector<kjm::_tstring> get_ExcludeList() const { return m_excludeList; }
	void set_ExcludeList(const std::vector<kjm::_tstring>& newValue) { m_excludeList = newValue; }
...
private:
	kjm::_tstring m_strOrg;					// 原本のパス
	kjm::_tstring m_strMir;					// 複製のパス
	modeOption m_mode;						// 動作モード
	bool m_fTest;							// テストモード時 true
	DWORD m_warningSec;						// 警告を表示する秒数
	std::vector<kjm::_tstring> m_excludeList;	// 除外ファイル一覧
