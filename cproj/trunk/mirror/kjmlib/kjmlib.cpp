...
kjm::_tstring kjm::util::get_environment_variable(const kjm::_tstring& lpName) {
	...
	if (dwRet == 0) {
		return kjm::_tstring();	// 環境変数なしの時は、空文字列を返す
	}
...
//---------------------------------------------------------------------
kjm::_tstring kjm::util::path_rename_extension(const kjm::_tstring& pszPath, const kjm::_tstring& pszExt) {
	TCHAR buffer[MAX_PATH];
	if (PathRenameExtension(lstrcpy(buffer, pszPath.c_str()), pszExt.c_str()) == FALSE) {
		return kjm::_tstring();	// 関数失敗の時は、空文字列を返す
	}
...
void kjm::util::cleanup_dir(const kjm::_tstring& pszPath, int nDay) {
	...
	do {
		if ((data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0) {
			...
			if (diff >= nDay) {
				kjm::_tstring delFile = kjm::util::path_append(pszPath, data.cFileName);
						  ...
						  DWORD dwCopyFlags)
{
	...
	kjm::_tstring existFile = lpExistingFileName;
	if (existFile.length() >= MAX_PATH) {
		existFile = kjm::path::makeVeryLongPath(existFile);
	}

	kjm::_tstring newFile = lpNewFileName;
	if (newFile.length() >= MAX_PATH) {
		newFile = kjm::path::makeVeryLongPath(newFile);
	}

	_tprintf(_T("-----\n"));
	_tprintf(_T("[%d] %s\n[%d] %s\n"), existFile.length(), existFile.c_str(), newFile.length(), newFile.c_str());

	// コピー元ファイルのファイル情報を取得しておく
	kjm::fileInfo fi(existFile);
...
// LoadString 関数のラッパー
//---------------------------------------------------------------------
kjm::_tstring kjm::util::loadString(UINT uID) {
...
// LoadString関数のラッパー
//---------------------------------------------------------------------
kjm::_tstring kjm::util::load_string(UINT uID, HINSTANCE hInstance) {
	std::vector<TCHAR> buffer(512);
	while (1) {
		int nRet = LoadString(hInstance, uID, &buffer[0], buffer.size());
		if (nRet == 0) {
			return kjm::_tstring();	// 文字列リソースが無い時は、空文字列を返す
...
//---------------------------------------------------------------------
kjm::_tstring kjm::util::getModuleFileName(HMODULE hModule/*=NULL*/) {
...
kjm::_tstring kjm::util::getMscVerText(int mscVer/* = _MSC_VER*/) {
...
// ディレクトリを再帰的に削除する
//---------------------------------------------------------------------
void kjm::util::removeFolder(const kjm::_tstring& pszPath) {
	std::list<kjm::_tstring> targetFolder;
...
std::vector<std::basic_string<TCHAR> > kjm::util::split(const std::basic_string<TCHAR>& src, TCHAR c) {
	std::basic_istringstream<TCHAR> iss(src);
...
std::wstring kjm::util::toString(const std::vector<wchar_t>& data) {
	...
	if (work[0] == 0xfffe) {
		...
		for (std::vector<wchar_t>::size_type i = 0; i < work.size(); i++) {
		...
		if (argv[i][0] == '/' || argv[i][0] == '-') {
			...
			if (argv[i][1] == '/' || argv[i][1] == '-') {
				...
				if (p != &argv[i][lstrlen(argv[i])]) {
					m_optList[kjm::_tstring(&argv[i][2], p)] = (p + 1);
				} else {
				...
				if (std::find(&hasArgShortOpts[0], &hasArgShortOpts[lstrlen(hasArgShortOpts)], argv[i][1]) != &hasArgShortOpts[lstrlen(hasArgShortOpts)]) {
					...
					if (argv[i][2] != '\0') {
						...
						m_optList[kjm::_tstring(1, argv[i][1])] = &argv[i][2];
					} else if (((i + 1) < argc) && (argv[i + 1][0] != '-' && argv[i + 1][0] != '/')) {
						// オプションの次のオプションがオプション文字列で始まっていないとき、
						// それを、オプションの引数として採用する
						m_optList[kjm::_tstring(1, argv[i][1])] = argv[i + 1];
						++i;
					} else {
						// オプションの引数は無かった
						m_optList[kjm::_tstring(1, argv[i][1])] = _T("");
					}
				} else {
					m_optList[kjm::_tstring(1, argv[i][1])] = _T("");
				}
...
// 日時をフォーマットして出力
//---------------------------------------------------------------------
kjm::_tstring kjm::dateTime::toString(const kjm::_tstring& format) const {
...
// で置換し、置換後の文字列全体を返します。
//---------------------------------------------------------------------
kjm::_tstring kjm::environment::expandEnvironmentVariables(const kjm::_tstring& name) {
	std::vector<TCHAR> work(MAX_PATH);
...
//---------------------------------------------------------------------
bool kjm::cfile::open(const kjm::_tstring& fname, const kjm::_tstring& mode, int shflag) {
	if (m_fp != NULL) {
		this->close();
	}
	...
	if (shflag == 0) {
		...
		m_fp = _tfsopen(fname.c_str(), mode.c_str(), shflag);
	}
...
// ファイルを削除する
//---------------------------------------------------------------------
void kjm::file::deleteFile(const kjm::_tstring& path, bool force) {
...
DWORD kjm::file::getAttributes(const kjm::_tstring& path) {
	kjm::_tstring work = path;
	if (work.length() >= MAX_PATH) {
		work = kjm::path::makeVeryLongPath(work);
	}

	DWORD dwAttr = ::GetFileAttributes(work.c_str());
...
//---------------------------------------------------------------------
kjm::fileInfo::fileInfo(const kjm::_tstring& strParentPath, const WIN32_FIND_DATA& rFindData) {
...
// ファイル名で初期化するコンストラクタ
//---------------------------------------------------------------------
kjm::fileInfo::fileInfo(const kjm::_tstring& strName) {
	m_fullPath = strName;

	kjm::_tstring work = (m_fullPath.length() >= MAX_PATH ? kjm::path::makeVeryLongPath(m_fullPath) : m_fullPath);

	findFile ff;
	if (ff.findFirstFile(work)) {
...
// ドライブレターを指定するコンストラクタ
//---------------------------------------------------------------------
kjm::drive::drive(const kjm::_tstring& driveLetter) {
...
kjm::log::log() {
	...
	kjm::_tstring strFileSpec = kjm::path::findFileName(kjm::util::getModuleFileName());
...
// ログをシフトする
//---------------------------------------------------------------------
void kjm::log::shift(const kjm::_tstring& fname) {
...
// ログファイルの出力先を設定する
//---------------------------------------------------------------------
void kjm::log::set_log_path(const kjm::_tstring& strPath) {
	// 出力先フォルダを作成しておく
...
// オプション情報指定付きコンストラクタ
kjm::optinfo::optinfo(TCHAR shortOption, const kjm::_tstring& longOption, kjm::optarg_requirement flag) {
...
kjm::optinfo::optinfo(const kjm::_tstring& longOption, kjm::optarg_requirement flag) {
	this->ShortOption = '\0';
	this->LongOption = longOption;
	this->OptArgRequirement = flag;
...
kjm::optinfo& kjm::optparse::findOption(TCHAR shortOpt) {
	...
	throw kjm::invalidOption(kjm::encoding::to_mbs(_T("-") + kjm::_tstring(1, shortOpt)));
}

// ロングオプションを指定したオプション情報を探す
kjm::optinfo& kjm::optparse::findOption(const kjm::_tstring& longOpt) {
...
kjm::optinfo& kjm::optparse::operator[](const kjm::_tstring& longOpt) {
	return findOption(longOpt);
}

// オプションの解析
kjm::optparse::ARGS kjm::optparse::parse(int argc, TCHAR** argv) {
	std::list<kjm::_tstring> args;
	for (int i = 1; i < argc; i++) {
		if (argv[i][0] == '-') {
			TCHAR c = argv[i][1];
			if (c == '-') {
				// ロングオプションの処理
				kjm::_tstring work(&argv[i][2]);
				kjm::_tstring::size_type pos = work.find_first_of('=');
				kjm::_tstring option_name = kjm::_tstring(work, 0, pos);

				optinfo& opt = findOption(option_name);
				opt.HasOption = true;
				if (opt.OptArgRequirement == required_argument) {
					if (pos != kjm::_tstring::npos) {
						opt.OptArg = kjm::_tstring(work, pos + 1);
					} else {
						// 引数のオプションが指定されていない！
				...
				if (opt.OptArgRequirement == required_argument) {
					if (argv[i][2] != '\0') {
						...
						// 引数のオプションが指定されていない！
						throw kjm::missingArgument(kjm::encoding::to_mbs(_T("-") + kjm::_tstring(1, c)));
					}
				} else if (opt.OptArgRequirement == optional_argument) {
					if (argv[i][2] != '\0') {
						// ショートオプションに続いてオプションの引数が書いてある時
