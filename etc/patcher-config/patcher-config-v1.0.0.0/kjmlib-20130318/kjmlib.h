...
namespace kjm {
	...
	class equal_char_ignorecase {
	public:
		bool operator()(TCHAR x, TCHAR y) const {
			return _totlower(x) == _totlower(y);
		}
	};

	bool icmp(const _tstring& x, const _tstring& y);
...
#define DTK_LOCAL		2	// DateTimeKind.Local 相当
	...
	public:
		...
		// オプションがあったかどうか調べる
		bool hasKey(const _tstring& key) const {
			return (m_optList.find(key) != m_optList.end());
		}

		// オプションの引数を取得する
		_tstring getOptArg(const _tstring& key) const {
			return hasKey(key) ? m_optList.find(key)->second : _T("");
		}
	...
	public:
		...
		// セクションデータを取得
		_tstrings getSectionData2(const _tstring& section) const;
