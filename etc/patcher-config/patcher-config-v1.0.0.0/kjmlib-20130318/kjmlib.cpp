...
bool kjm::icmp(const _tstring& x, const _tstring& y) {
	return (x.size() == y.size()) && (std::equal(x.begin(), x.end(), y.begin(), kjm::equal_char_ignorecase()));
}
						  ...
						  DWORD dwCopyFlags)
{
	...
	if (result) {
		FILETIME ft;
		if (GetFileTime(hSourceFile, NULL, NULL, &ft))
...
std::basic_string<TCHAR> kjm::path::combine(const std::basic_string<TCHAR>& path1, const std::basic_string<TCHAR>& path2) {
	...
	if (!::PathAppend(::lstrcpy(&work[0], path1.c_str()), path2.c_str())) {
		DWORD dwErr = GetLastError();
		_otstringstream oss;
		oss << _T("kjm::path::combine: path名 '")
			<< path1
			<< _T("' に、path名 '")
			<< path2
			<< _T("' を結合できませんでした。") << kjm::util::formatMessageBySystem(dwErr);
		throw kjm::exception::exception(dwErr, oss.str());
	}
...
int kjm::cmdLine::chkarg(int argc, TCHAR **argv, const kjm::_tstring &shortOpts) {
	...
	for (int i = 0; i < (int)shortOpts.length(); i++) {
		int optArg = no_argument;
		TCHAR c = shortOpts[i];
		if (((i + 1) < (int)shortOpts.length()) && (shortOpts[i + 1] == _T(':'))) {
...
// 日時をフォーマットして出力
//-----------------------------------------------------------------------------
kjm::_tstring kjm::dateTime::toString(const _tstring& format) {
	SYSTEMTIME st;
	::FileTimeToSystemTime(&m_ft, &st);

	struct tm tmBuf;
	memset(&tmBuf, 0, sizeof(tmBuf));
	tmBuf.tm_sec = st.wSecond;			// 秒
	tmBuf.tm_min = st.wMinute;			// 分
	tmBuf.tm_hour = st.wHour;			// 時
	tmBuf.tm_mday = st.wDay;			// 日
	tmBuf.tm_mon = st.wMonth - 1;		// 月
	tmBuf.tm_year = st.wYear - 1900;	// 年
...
// セクションデータを取得
//-----------------------------------------------------------------------------
kjm::_tstrings kjm::iniFile::getSectionData2(const kjm::_tstring& section) const {
	kjm::_tstrings result;
	std::vector<TCHAR> data = kjm::iniFile::getSectionData(section);

	std::vector<TCHAR>::iterator top = data.begin();
	while (1) {
		std::vector<TCHAR>::iterator pos = std::find(top, data.end(), _T('\0'));
		kjm::_tstring key_value = kjm::_tstring(top, pos);
		if (key_value.empty()) {
			break;
		}
...
void kjm::log::set_log_path(const _tstring& strPath) {
	_tstring work = kjm::util::removeBackslash( strPath );
	//_tstring work = strPath;
	// 出力先フォルダを作成しておく
	try {
		kjm::directory::createDirectory( work );
	} catch (kjm::exception e) {
		_ftprintf(stderr, _T("error %d: %s"), e.m_errCode, e.m_errMsg.c_str());
		// ログが作れないだけなので、とりあえず先に進める。
	}

#if defined(USE_WSSLOG)
#if defined(UNICODE)
	WLSetLogPath(kjm::util::toAnsi( work ).c_str());
#else
