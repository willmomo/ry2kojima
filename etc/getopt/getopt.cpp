      ...
      if (nonoption_flags_max_len == 0)
	{
	  if (__getopt_nonoption_flags == NULL
	  ...
	  else
	    {
	      ...
	      int len = nonoption_flags_max_len = _tcslen (orig_str);
...
int _getopt_internal (int argc, TCHAR *const *argv, const TCHAR *optstring, const struct option *longopts, int *longind, int long_only)
{
  ...
  if (longopts != NULL
      ...
      if (pfound != NULL)
	{
	 ...
	 Otherwise interpret it as a short option.  */
	  ...
	  || _tcschr (optstring, *nextchar) == NULL)
	{
	  if (print_errors)
	    {
	      if (argv[optind][1] == '-')
