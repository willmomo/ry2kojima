...
BOOL CKeyhookDlg::OnInitDialog()
{
	...
	SetHook(m_hWnd);
	RegisterHotKey(m_hWnd, ID_DISABLE_ALTESC, MOD_ALT, VK_ESCAPE);
	RegisterHotKey(m_hWnd, ID_DISABLE_CTRLALTESC, MOD_ALT|MOD_CONTROL, VK_ESCAPE);
...
void CKeyhookDlg::OnDestroy() 
{
	...
	UnregisterHotKey(m_hWnd, ID_DISABLE_ALTESC);
	UnregisterHotKey(m_hWnd, ID_DISABLE_CTRLALTESC);
...
LRESULT CKeyhookDlg::OnKeyHook(WPARAM wParam, LPARAM lParam)
{
	...
	char szBuff[256];
	wsprintf(szBuff, "wParam = 0x%02X, lParam = 0x%02X", wParam, lParam);
