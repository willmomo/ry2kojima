...
/* set_sys:  */
 ...
 /*   00000.0   97.05.21     y.I       新規作成        */             /* 直接書込データ:  */
 /*   構造体ﾂｰﾙにて再作成                              */             /* 直接書込データ:  */
 /*  1998/10/9 12:06 osako             */          /* 直接書込データ:  */
 /* WAIS START */
...
typedef struct _fno1000 {
	...
	unsigned short daigotocard;	/* 台毎カード端末接続　*/		// 台毎カード端末追加  Fukuyama	
//    unsigned char yobi2[30];    /* 予備:  */
    unsigned char yobi2[28];    /* 予備:  */					// 台毎カード端末  Fukuyama
    unsigned short tenposu;     /* 店舗数 */
    ...
    unsigned short option8;     /* オプション８: 台ファイルの売上補正値の単位設定   */
                                /*  ＝０  １円単位                                  */
                                /*  ≠０  １００円単位                              */
#if	1	/* 2002.04.04(thu) chg.By Koketsu For Winsousa */
...
typedef struct _fno1008 {
	unsigned char	tnpstrAB[12];		/* 全店合算名 */
	unsigned char	tnpstr_A[12];		/* 店舗名１ */
	unsigned char	tnpstr_B[12];		/* 店舗名２ */
	unsigned char	dmy1[36];			/* 予備１ */
	unsigned char	sbtstra1[12];		/* 種別名１ */
	unsigned char	sbtstra2[12];		/* 種別名２ */
	unsigned char	sbtstra3[12];		/* 種別名３ */
	unsigned char	sbtstra4[12];		/* 種別名４ */
	unsigned char	sbtstra5[12];		/* 種別名５ */
	unsigned char	sbtstra6[12];		/* 種別名６ */
	unsigned char	sbtstra7[12];		/* 種別名７ */
	unsigned char	sbtstra8[12];		/* 種別名８ */
	unsigned char	dmy2[24];			/* 予備２ */
} fno1008_t;
...
typedef fno1008_t FSET_SYS_TNPNAME;		// F1008 店舗･種別名称設定 ←2002/11/25 (店舗数対応)追加 Fukuyama
