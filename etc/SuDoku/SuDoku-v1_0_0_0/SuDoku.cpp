	...
	for (y = 0; y < 9; y++) {
		for (x = 0; x < 9; x++) {
			if (g_game.m_cells[y][x].m_fixed != 0) {
				...
				if (g_game.m_cells[y][x].m_sender == 2) {			// (青)候補がひとつになったとき
					hbrOld = (HBRUSH)SelectObject(hdc, hbr3);
					bkOld = SetBkColor(hdc, RGB(150, 150, 250));
				}
				if (g_game.m_cells[y][x].m_sender == 3) {			// (赤)横方向にひとつだけ候補があったとき
					hbrOld = (HBRUSH)SelectObject(hdc, hbr4);
					bkOld = SetBkColor(hdc, RGB(250, 150, 150));
				}
				if (g_game.m_cells[y][x].m_sender == 4) {			// (緑)縦方向にひとつだけ候補があったとき
