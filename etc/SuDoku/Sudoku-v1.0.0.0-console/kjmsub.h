...
namespace kjm {
	...
	******************************************************************/
	...
	public:
		...
		// ファイルが存在するかどうか調べる
		static bool fileExists(const _tstring& strPath) { return (PathFileExists(strPath.c_str()) != FALSE); }

		// フルパスから、ファイル名を取り出す
		static _tstring findFileName(const _tstring& strPath) { return PathFindFileName(strPath.c_str()); }

		// ディレクトリかどうか調べる
		static bool isDirectory(const _tstring& strPath) { return (PathIsDirectory(strPath.c_str()) != FALSE); }
	...
	public:
		...
		// ファイルの属性を取得する。
		static kjm::fileAttributes getAttributes(const _tstring &path);

		// ファイルの属性を設定する。
		static void setAttributes(const _tstring& path, kjm::fileAttributes fa);
	};

	//-------------------------------------------------------------------------
	// ディレクトリの一般的な操作を提供するクラス
	//-------------------------------------------------------------------------
	class directory {
	public:
		// ディレクトリを作成する。
