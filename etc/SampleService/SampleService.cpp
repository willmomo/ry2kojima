...
VOID WINAPI SntpdServiceMain(DWORD dwArgc, LPTSTR* lpszArgv) {
	...
	_tstring logPath = kjm::util::get_private_profile_string(_T("option"), _T("logpath"),
		kjm::util::path_append(kjm::util::get_environment_variable(_T("TEMP")).c_str(), _T("sntpd")),
		kjm::util::path_rename_extension(kjm::util::get_module_file_name().c_str(), _T(".ini")).c_str());
...
int _tmain(int argc, TCHAR** argv) {
	if (argc != 1) {
		if (lstrcmp(argv[1], _T("install")) == 0) {
			install(kjm::util::get_module_file_name().c_str());
	...
	if (!StartServiceCtrlDispatcher(services)) {
		MessageBox(NULL, _T("sntpdサービスでエラーが発生しました。"), _T("sntpdエラー"), MB_OK | MB_ICONERROR | MB_SERVICE_NOTIFICATION);
	}
