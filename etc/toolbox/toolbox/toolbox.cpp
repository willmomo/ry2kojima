...
//---------------------------------------------------------------------
// toolbox.command をチェックして処理を行う。
//
// 1分に1回のWM_TIMERで呼び出してもらう。
//---------------------------------------------------------------------
static void CheckCommand( HWND hwnd ) {

	// toolbox.command をチェック!
	Winssa_Log( LOGC_NAME, LOGLV_DEBUG, _T( "コマンドファイルの存在をチェックします。" ) );

	// 自身のファイル名から、拡張子を command に変える
	kjm::path_string cmd_file = kjm::path_string::GetModuleFileName();
	cmd_file.RenameExtension( _T( ".command" ) );

	// ファイルが存在する?
	if ( cmd_file.FileExists() ) {

		// コマンドファイルがあったので終了する
		Winssa_Log( LOGC_NAME, LOGLV_INFO, _T( "%s を発見。終了します。" ), cmd_file.c_str() );

		// ファイルを消す
		remove( cmd_file.c_str() );
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow ) {
	...
	// 開始ログ
	Winssa_Log( LOGC_NAME, LOGLV_ALWAYS, _T( "[ %s ] start." ), kjm::path_string::GetModuleFileName().c_str() );
	...
	// 終了ログ
	Winssa_Log( LOGC_NAME, LOGLV_ALWAYS, _T( "[ %s ] end. (%d)" ), 
		kjm::path_string::GetModuleFileName().c_str(), msg.wParam );
