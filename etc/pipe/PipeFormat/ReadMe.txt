========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : PipeFormat
========================================================================


AppWizard が生成した PipeFormat には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには PipeFormat を構成している各ファイルの概要説明が含まれています。
...
PipeFormat.h
    ...
    みます） をインクルードし、また CPipeFormatApp アプリケーションクラスを宣
    言します。

PipeFormat.cpp
    このファイルは CPipeFormatApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\PipeFormat.ico
    ...
    ルです。このアイコンはリソース ファイル PipeFormat.rc によってインクルード
    されます。
...
PipeFormatDlg.h, PipeFormatDlg.cpp - ダイアログ
    これらのファイルは CPipeFormatDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な PipeFormat.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) PipeFormat.pch 
