...
BOOL MainDlg_Button3_OnClick(HWND hwnd, HWND hwndCtl) {
	...
	EnableWindow(GetDlgItem(hwnd, IDC_GENERIC_READ), TRUE);
	EnableWindow(GetDlgItem(hwnd, IDC_GENERIC_WRITE), TRUE);
...
BOOL MainDlg_Button2_OnClick(HWND hwnd, HWND hwndCtl) {
	...
	if (Button_GetCheck(GetDlgItem(hwnd, IDC_GENERIC_READ))  == BST_CHECKED) dwDesiredAccess |= GENERIC_READ;
	if (Button_GetCheck(GetDlgItem(hwnd, IDC_GENERIC_WRITE)) == BST_CHECKED) dwDesiredAccess |= GENERIC_WRITE;
	...
	g_hFile = CreateFile(fileName, dwDesiredAccess, dwShareMode, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (g_hFile != INVALID_HANDLE_VALUE) {
		EnableWindow(GetDlgItem(hwnd, IDC_BUTTON2), FALSE);
		EnableWindow(GetDlgItem(hwnd, IDC_BUTTON3), TRUE);

		EnableWindow(GetDlgItem(hwnd, IDC_GENERIC_READ), FALSE);
		EnableWindow(GetDlgItem(hwnd, IDC_GENERIC_WRITE), FALSE);
...
BOOL MainDlg_Size(HWND hwnd, WPARAM wParam, LPARAM lParam) {
	...
	kjmutil::rect rcDesiredAccess = kjmutil::rect::get_window_rect(GetDlgItem(hwnd, IDC_DESIRED_ACCESS)).screen_to_client(hwnd);
	MoveWindow(GetDlgItem(hwnd, IDC_DESIRED_ACCESS),
		rcDesiredAccess.left,
		rcDesiredAccess.top,
		rcClient.get_width() - (12 * 2),
		rcDesiredAccess.get_height(),
		TRUE);

	kjmutil::rect rcShareMode = kjmutil::rect::get_window_rect(GetDlgItem(hwnd, IDC_SHARE_MODE)).screen_to_client(hwnd);
	MoveWindow(GetDlgItem(hwnd, IDC_SHARE_MODE),
		rcShareMode.left,
		rcShareMode.top,
		rcClient.get_width() - (12 * 2),
		rcShareMode.get_height(),
		TRUE);
