...
//=============================================================================
// パフォーマンスデータを処理するクラス(class cpuUsage)
//=============================================================================
//-----------------------------------------------------------------------------
// パフォーマンスデータ取得開始(95版)
//-----------------------------------------------------------------------------
...
//---------------------------------------------------------------------
// レイヤードウィンドウの不透明度と透明のカラーキーを設定
//
// hwnd     レイヤードウィンドウのハンドルを指定します。レイヤードウィ
//          ンドウは、CreateWindowEx 関数に WS_EX_LAYERED を指定して直
//          接作成するか、SetWindowLong 関数に WS_EX_LAYERED を指定して
//          既存のウィンドウから作成します。
// crKey    レイヤードウィンドウの作成時に使う透明のカラーキーを指定す
//          る構造体へのポインタを指定します。ウィンドウによって描画さ
//          れるこの色のピクセルはすべて透明になります。
// bAlpha   レイヤードウィンドウの不透明度を示すアルファ値を指定します。
//          構造体の SourceConstantAlpha メンバに似ています。0 を指定す
//          ると、ウィンドウは完全に透明になります。255 を指定すると、
//          ウィンドウは不透明になります。
// dwFlags  アクションフラグを指定します。次の値を組み合わせて使います。
//          値            意味 
//          LWA_COLORKEY  透明色として crKey を使います。 
//          LWA_ALPHA     bAlpha を使って、レイヤードウィンドウの不透明
//                        度を決定します。
//---------------------------------------------------------------------
