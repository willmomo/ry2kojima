...
bool CSoundData::Stop()
{
...

	{
		...
		mciSetParms.dwAudio = MCI_SET_AUDIO_ALL;
		MCIERROR mcierror = mciSendCommand(m_deviceID, MCI_SET, MCI_WAIT | MCI_SET_AUDIO | MCI_SET_OFF, (DWORD)&mciSetParms);
		if (mcierror == 0) {
			m_mciLastErrorText = "";
		} else {
			char errText[256];
...
//	MCIERROR mcierror = mciSendCommand(m_deviceID, MCI_STOP, MCI_WAIT, 0);
	{
//		MCIERROR mcierror = mciSendCommand(m_deviceID, MCI_PAUSE, 0, 0);
	}
	MCIERROR mcierror = mciSendCommand(m_deviceID, MCI_STOP, 0, 0);
	if (mcierror == 0) {
		m_mciLastErrorText = "";
	} else {
		char errText[256];
	...
	{
		mciSendCommand(m_deviceID, MCI_SEEK, MCI_SEEK_TO_START,0);
	}
