...
// 送信する
//-----------------------------------------------------------------------------
int kjm::baseSocket::send(const std::vector<char>& buf) {
	int ret = ::send(this->m_sock, &buf[0], buf.size(), 0);
	this->m_lastError = ::WSAGetLastError();
	return ret;
}
...
std::basic_string<TCHAR> kjm::path::combine(const std::basic_string<TCHAR>& path1, const std::basic_string<TCHAR>& path2) {
	std::vector<TCHAR> work(MAX_PATH);
	::PathAppend(::lstrcpy(&work[0], path1.c_str()), path2.c_str());
	return &work[0];
}
...
//-----------------------------------------------------------------------------
kjm::_tstring kjm::path::getTempFileName(const kjm::_tstring& prefixString, const kjm::_tstring& pathName/*=kjm::path::getTempPath()*/, UINT uUnique/*=0*/) {
	kjm::_tstring result;

	std::vector<TCHAR> work(MAX_PATH);
	if (::GetTempFileName((pathName.empty() ? NULL : pathName.c_str()), prefixString.c_str(), uUnique, &work[0]) != 0) {
		result = &work[0];
	}
...
//-----------------------------------------------------------------------------
kjm::_tstring kjm::path::getTempPath() {
	std::vector<TCHAR> path(MAX_PATH);
...
// note:
//-----------------------------------------------------------------------------
bool kjm::mutex::create(const std::basic_string<TCHAR>& name) {
...
// 埋込み文字列の取得
//-----------------------------------------------------------------------------
int kjm::eventLogRecord::GetArgs(const EVENTLOGRECORD *pBuf, std::vector<char*>& args )
{
	args.clear();
...
// ソース名からモジュール名を取得
//-----------------------------------------------------------------------------
BOOL kjm::eventLogRecord::GetModuleNameFromSourceName( const char *SourceName, const char *EntryName, std::vector<TCHAR>& ExpandedName) {
	DWORD lResult;
...
// メッセージの表示
//-----------------------------------------------------------------------------
std::string kjm::eventLogRecord::DispMessage( const char *SourceName, const char *EntryName, const char **Args, DWORD MessageId) {
	BOOL bResult;
	BOOL bReturn = FALSE;
	...
	oneModuleName = strtok( &SourceModuleName[0], ";" );
	while ( oneModuleName ) {
	...
	if(message.size() > 0) return std::string(&message[0]);
	else {
		char buf[32];
		wsprintf(buf, "(%d)\n", MessageId);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
std::string kjm::eventLogRecord::ComputerName() {
	char* cp = (char *)getRaw();
...
// 文字列で時間を指定するコンストラクタ
//-----------------------------------------------------------------------------
void kjm::time::parse( const char* time_text ) {

	int y, m, d, h, n, s;
	int ret = sscanf( time_text, "%d/%d/%d %d:%d:%d", &y, &m, &d, &h, &n, &s );
