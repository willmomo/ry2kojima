...
void GetIniFile() {
	...
	{
		char temp[MAX_PATH];
		PathAppend(strcpy(temp, m_BasePath), cLogPath);
		WLSetLogPath(temp);
	}

	// ログファイル名の形
	m_logNameType = GetPrivateProfileInt("Log", "LogNameType", 1, cPath);

	// LogLevelの取得
	m_nLogLevel = GetPrivateProfileInt("Log", "LogLevel", 2, cPath);

	// 保存日数の取得
	m_nSaveDays = GetPrivateProfileInt("Log", "SaveDays", DEFAULT_LOG_SAVEDAYS, cPath);

	// ミューテックス名を作成する。(C:\temp\temp.exe → mtx_C:/temp/)
	// ミューテックス名　"mtx_" + m_BasePath
	strcat(strcpy(m_MTX_SYSLOG, "mtx_"), m_BasePath);
...
void WINAPI Winssa_Log(const char *sikibetu,long loglevel,const char * format, ...) {
	...
	// スレッドモードの時は、ログ文字列を作成して、queueにpushする。
	{
		...
		sprintf(buf,"%02d:%02d:%02d.%03d [%-6s]{%04X:%04X}:",
