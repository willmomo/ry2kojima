...
// 削除処理
//-----------------------------------------------------------------------------
bool doDelete(const kjm::cmdLine& cl) {
	kjm::_tstring fname = cl.getOptArgument(_T("file"));
	kjm::_tstring windowTitle = cl.getOptArgument(_T("window-title"));

	if (fname.empty() || windowTitle.empty()) {
		_tcout << _T("削除モードのときは、--file, --window-title を必ず指定する必要があります。\n");
		return false;
	}
	...
	if (wp > 0) {	// 削除するセクションが見つかった
		kjm::_tstring windowTitle;
		do {
			oss.str(_T(""));
			oss << _T("WATCH") << rp;
			kjm::_tstring rSec = oss.str();

			oss.str(_T(""));
			oss << _T("WATCH") << wp;
			kjm::_tstring wSec = oss.str();

			windowTitle = ini.getString(rSec, _T("WINDOW_TITLE"));

			ini.writeString(wSec, _T("WINDOW_TITLE"), windowTitle);
			ini.writeString(wSec, _T("PROGRAM"), ini.getString(rSec, _T("PROGRAM")));
			ini.writeString(wSec, _T("EXECPATH"), ini.getString(rSec, _T("EXECPATH")));

			++rp;
			++wp;
		} while (!windowTitle.empty());
	}
...
int _tmain(int argc, TCHAR** argv) {
	...
	options.push_back(kjm::optionInfo(_T("delete"), _T("d"), kjm::no_argument, _T("削除モードで実行")));
	...
	if (cl.hasOption(_T("append"))) {
		...
		// 表示処理を実行
		doView(cl);
	} else if (cl.hasOption(_T("delete"))) {
		// 削除処理を実行
		doDelete(cl);
	} else {
		usage(options);
