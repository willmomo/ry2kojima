...
unsigned char p_send_buf[1024];				/* POS送信手順用ﾊﾞｯﾌｧ */
unsigned char h_send_buf[1024];				/* MPC送信手順用ﾊﾞｯﾌｧ */
...
int csv_read_func( void )
{
	...
	memset(p_send_buf, NULL, sizeof(p_send_buf));	/* p_send_bufの初期化 */
	memset(h_send_buf, NULL, sizeof(h_send_buf));	/* h_send_bufの初期化 */
		...
		for(j = 0; j < i; j++){							/* サイズ分ループ */
			...
			if ( (lbuf[j] == ',') || (lbuf[j] == 0x0a) || (lbuf[j] == 0x0d) ) {	/* [,]と改行コード */
				...
				if (len != 0) {							/* データ長が0以外 */
						...
						if(data == SKIP){
							...
							if(pos_send == TRUE){
								if(loop_flag == LOOP_ED){
									...
									for(k=0;k<loop_cnt;k++){
										memcpy(&(p_send_buf[pcnt]), &(p_send_loop_buf[0]), plcnt);
										pcnt += plcnt;
									}
									plcnt = 0;
									loop_flag = LOOP_NONE;
							...
							if(harai_send == TRUE){
								if(loop_flag == LOOP_ED){
									/* コピーしたものを挿入 */
									for(k=0;k<loop_cnt;k++){
										memcpy(&(h_send_buf[hcnt]), &(h_send_loop_buf[0]), hlcnt);
										hcnt += hlcnt;
									}
									hlcnt = 0;
									loop_flag = LOOP_NONE;
					...
					if(pos_send == TRUE){
						/* POSの送信手順ﾊﾞｯﾌｧへｺﾋﾟｰ */
						memcpy(&(p_send_buf[pcnt]), &data, sizeof(char));			/* p_bufへのデータのコピー */
					...
					if(harai_send == TRUE){
						/* MPCの送信手順ﾊﾞｯﾌｧへｺﾋﾟｰ */
						memcpy(&(h_send_buf[hcnt]), &data, sizeof(char));			/* h_bufへのデータのコピー */
						if(loop_flag == LOOP_ST){
							/* コピー開始 */
...
int send_mtanka( char *data )
{
	...
	/* CMD番号を変える */
	tanka_check_mars[1] = '5';
	tanka_check_mars[2] = '2';
	/* BCCを計算 */
	for(i = 0; i < sizeof(tanka_check_mars) - 5; i++){
		bcc += tanka_check_mars[i];
	}
	bcc = 0xff - (char)bcc;
	bcc1 = ((bcc & 0xf0) >> 4) + '0';		/* 1BYTE分をASCII変換 */
	if((bcc1 >= 0) && (bcc1 <= 9)){
		/* 0～9 */
		bcc1 += '0';
	}else{
	...
	tanka_check_mars[5+30+1] = bcc1;
	tanka_check_mars[5+30+2] = bcc2;
...
int send_status( char mode )
{
	...
	case H_MARS:
		...
		check_mars[5+30+1] = bcc1;
		check_mars[5+30+2] = bcc2;
...
/* 																			 */
/*****************************************************************************/
void make_harai_mars( char *data )
{
	int sum_tama;		/* 総玉数 */
