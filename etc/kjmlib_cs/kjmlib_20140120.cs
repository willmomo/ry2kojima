...
namespace Kjm {
	public class Log {
		...
		public static string LogPath {
			...
			set { Log.s_path = System.IO.Path.GetFullPath(System.Environment.ExpandEnvironmentVariables(value)); }
		}
	...
	public class Util {
		...
		public static void CreateShortCut(String path, String targetPath, String Arguments, String WorkingDirectory, String description) {
			//using System.Reflection;
		...
		/// <returns></returns>
		public static int Run(string cmd, string arg, ProcessWindowStyle style, bool wait) {
			int result = -1;
		...
		public static int Run(string cmd) {
			return Run(cmd, "", ProcessWindowStyle.Normal, true);
		}

		public static int RunNoWait(string cmd) {
			return Run(cmd, "", ProcessWindowStyle.Normal, false);
		}
		...
		/// <returns></returns>
		/// <example>ReplaceEx(org, new []{"%TITLE%", "%VERSION%"}, new []{"タイトル", "v0.998"});</example>
		public static string ReplaceEx(string orgString, string[] oldValues, string[] newValues) {
			if (oldValues.Length != newValues.Length) {
				throw new ArgumentException("oldValuesとnewValuesの数が不一致");
			}
			for (int i = 0; i < oldValues.Length; i++) {
				orgString = orgString.Replace(oldValues[i], newValues[i]);
			}
			return orgString;
		}

		/// <summary>
		/// 実行ファイル名からINIファイル名を生成
		/// </summary>
		/// <returns></returns>
		public static string GetExeIniName() {
			return Path.ChangeExtension(System.Windows.Forms.Application.ExecutablePath, ".ini");
		}
	}
	...
	public class IniFile {
		...
		public void WriteValue(string section, string key, string newValue) {
