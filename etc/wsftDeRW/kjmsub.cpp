...
// オプション一覧を取得する
//-----------------------------------------------------------------------------
std::vector<_tstring> kjm::commandLine::getOptList() const {
	std::vector<_tstring> result;
	std::map<_tstring, _tstring>::const_iterator i;
...
// オプションの引数を取得する
//-----------------------------------------------------------------------------
_tstring kjm::commandLine::getOptArg(const _tstring& key) const {
	std::map<_tstring, _tstring>::const_iterator i = m_optList.find(key);
	return (i != m_optList.end()) ? i->second : _T("");
}
...
#else
	Winssa_Log(get_Ident().c_str(), LOGLV_ALWAYS, _T("%s"), &buf[0]);
#endif
#endif
}
...
#else
	Winssa_Log(get_Ident().c_str(), LOGLV_ERR, _T("%s"), &buf[0]);
#endif
#endif
}
...
#else
	Winssa_Log(get_Ident().c_str(), LOGLV_INFO, _T("%s"), &buf[0]);
#endif
#endif
}
...
#else
	Winssa_Log(m_strIdent.c_str(), LOGLV_INFO, _T("%s"), &work[0]);
#endif
#else
	va_list marker;
	va_start(marker, format);
