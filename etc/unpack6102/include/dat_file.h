...
#define TEIJISBT_RSUU		272			// 定時種別
#define TEIJIKISHU_RSUU		3264		// 定時機種
...
/*#**************************************/
/*	種別集計･ﾌﾛｱﾃﾞｰﾀ_EX
	(売上断線回数追加のため新規追加)　nakatani	*/
/****************************************/
typedef struct {
	uchar	hakken_dancnt;		/* ｻﾝﾄﾞ発券機断線回数		*/
	uchar	kinko_hakken_dancnt;/* 金庫券売機断線回数			*/
	uchar	kkdancnt;			/* 金庫断線回数（金庫券売機を含まず）*/
...
#if 1			/* 2004.1.8 nao */
	uchar	hakken_dancnt;	/* サンド発券断線回数 */
	uchar	kinko_hakken_dancnt;	/* 金庫券売機断線回数 */
	uchar	kkdancnt;		/* 金庫断線回数（金庫券売機断線含まず） */
	uchar	yobi[57];		/* ････				*/
#else
	uchar	yobi[60];		/* ....				*/
#endif
...
typedef	struct {
	ushort	daisu;			/* 台数				*/
	ushort	tcnt;			/* 特賞回数			*/
	ushort  renewalday;		/* 更新日付(営業日付参照): 年・月・日:bit7・4・5	←2004/1/14 (機種別定時稼動数変更 T2X000532対応)追加 Fukuyama	*/
	ushort  yobi[3];		/* 予備				*/
	ulong	urikin;			/* 売上(100円)			*/
	ulong	hosei;			/* 売上補正値(円)		*/
	ushort	kado;			/* 現稼動数			*/
...
// 2004/1/14 (機種別定時稼動数変更 T2X000532対応) 追加 Fukuyama -->
/*#********************************************/
/*	定時稼動取得済み情報(GetTeiji.datの先頭メンバ)	*/
/**********************************************/
typedef struct {
	ushort  lastrenew;		/* 最終更新日付: 年・月・日:bit7・4・5	*/
	ushort  yobi;			/* 予備				*/
} DATA_KADO_GETLOG;

/*#**************************************/
/*	定時稼動取得情報(GetTeiji.dat)			*/
/****************************************/
typedef struct {
	DATA_KADO_GETLOG	getlog[TEIJIKADO_RSUU];
	FDATA_KADO_INF		info[TEIJIKADO_RSUU];
	FDATA_KADO_SBT		sbt[TEIJISBT_RSUU];
	FDATA_KADO_ZEN		zen[TEIJIKADO_RSUU];
	FDATA_KADO_KISHU	kishu[TEIJIKISHU_RSUU];
}	GET_TEIJI;
//--> 2004/1/14 (機種別定時稼動数変更 T2X000532対応) 追加 Fukuyama
...
#if 1			/* chg 2004.1.8 nao */
	uchar	hakken_dancnt;		/* ｻﾝﾄﾞ発券断線回数		*/
	uchar	kinko_hakken_dancnt;/* 金庫券売断線回数			*/
	uchar	kkdancnt;			/* 金庫断線回数（金庫券売機を含まず）*/
#else
