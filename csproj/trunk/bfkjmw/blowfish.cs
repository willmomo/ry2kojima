...
namespace BlowFishCS {
	class BlowFish {
		...
		public string Decrypt_ECB(string ct) {
			byte[] dec_data = Decrypt_ECB(HexToByte(ct));
			byte pad = Buffer.GetByte(dec_data, dec_data.Length - 1);
			byte[] data = new byte[dec_data.Length - pad];
			Buffer.BlockCopy(dec_data, 0, data, 0, data.Length);

			//return Encoding.ASCII.GetString(data).Replace("\0", "");
			return Encoding.GetEncoding("sjis").GetString(data).Replace("\0", "");
		}
		...
		public byte[] Encrypt_ECB(byte[] pt) {
			byte pad = (byte)(8 - (pt.Length % 8));
			byte[] plainText = new byte[pt.Length + pad];
			Buffer.BlockCopy(pt, 0, plainText, 0, pt.Length);
			for (int i = 0; i < pad; i++) {
				Buffer.SetByte(plainText, pt.Length + i, pad);
			}

			return Crypt_ECB(plainText, false);
		}
