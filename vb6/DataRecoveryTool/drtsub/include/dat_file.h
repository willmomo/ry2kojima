/* WAIS START */
/* $Header: /usr/external/CVS/as-sh3/include/dat_file.h,v 1.10 2003/08/25 00:40:16 nisimura Exp $ */
/********************************************************************************************************/
/*	データファイル											*/
/* Version	Date		Programmer	Memo							*/
/* Ver.1110	2003.07.10	Kimura		･特賞履歴の最下段を最新履歴にする｡			*/
/*						   累計台ﾃﾞｰﾀの最終発生時刻を特賞1最終発生時刻とする｡	*/
/*						   ※ｺﾒﾝﾄを変更するのみ｡				*/
/* Ver.1110	2003.07.03	Kimura		･S1,S2入力追加｡						*/
/*						   累計台ﾃﾞｰﾀにS1ｶｳﾝﾄ､S2ｶｳﾝﾄ､S2後の特賞1回数を追加｡	*/
/*						   ※ｺﾒﾝﾄを変更するのみ｡				*/
...
typedef	struct {
	...
	ushort	start1;			/* ｽﾀｰﾄ1			*/
	ushort	start2;			/* ｽﾀｰﾄ2			*/
	ushort	start3;			/* S1ｶｳﾝﾄ			*//* Ver.1110 2003.07.03 chg : ｽﾀｰﾄ3→S1ｶｳﾝﾄ */
	ushort	t2start1;		/* 特賞2(T2)中ｽﾀｰﾄ1		*/
	ushort	t2start2;		/* 特賞2(T2)中ｽﾀｰﾄ2		*/
	ushort	t2start3;		/* S2ｶｳﾝﾄ			*//* Ver.1110 2003.07.03 chg : 特賞2(T2)中ｽﾀｰﾄ3→S2ｶｳﾝﾄ */
	ushort	t1start2;		/* S2後の特賞1回数		*//* Ver.1110 2003.07.03 chg : 特賞ｽﾀｰﾄ2回数→S2後の特賞1回数 */
	...
	uchar	tflg;			/* 特賞ﾌﾗｸﾞ					*/
					...
					/*	0x02 : T1				*/
					/*	0x04 : T2(CT)				*/
					/*	0x08 : 連続T1(T1->T1 or T1->T2->T1)	*/
					/*	0x10 : ラッキー				*/
					/*	0x20 : RB(レギュラーボーナス)		*/
	uchar	dmy2;			/* ....				*/
	uchar	e_hour;			/* 特賞1最終発生時刻(時)	*//* Ver.1110 2003.07.10 chg : 最終時刻(時)→特賞1最終発生時刻(時) */
	uchar	e_min;			/* 特賞1最終発生時刻(分)	*//* Ver.1110 2003.07.10 chg : 最終時刻(分)→特賞1最終発生時刻(分) */
