...
public class MainActivity extends Activity {
    ...
    private BluetoothReceiver m_BluetoothReceiver = null;
    private BroadcastReceiver m_headsetPlug = null;

    @Override
    protected void onDestroy() {
        if (m_headsetPlug != null) {
            unregisterReceiver(m_headsetPlug);
        }
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        registerReceiver(m_BluetoothReceiver, bluetoothIntentFilter);
        Log.d("VCVCVC", "Bluetooth 関連のレシーバーを登録");

		// ヘッドセットのプラグ状態を取得するレシーバを作成
        m_headsetPlug = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                ...
                if (isPlugged) {
                    Toast.makeText(getApplicationContext(), "VC: ヘッドセット ON です", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(getApplicationContext(), "VC: ヘッドセット OFF です", Toast.LENGTH_SHORT).show();
                }
            }
        };
        registerReceiver(m_headsetPlug, new IntentFilter(Intent.ACTION_HEADSET_PLUG));
