...
class SudokuBoard {
    ...
    // 確定セルの数字をそれぞれカウントする
    public Map<Integer, Integer> countFixedNum() {
        Map<Integer,Integer> fixedNum = new HashMap<Integer,Integer>();
        for (int row = 0; row < this.blockCellCount(); row++) {
            for (int col = 0; col < this.blockCellCount(); col++) {
                if (this.getCell(row,col).getNumber() != 0) {
                    if (fixedNum.get(this.getCell(row,col).getNumber()) == null) {
                        fixedNum.put(this.getCell(row, col).getNumber(), 1);
                    } else {
                        fixedNum.put(this.getCell(row, col).getNumber(), fixedNum.get(this.getCell(row,col).getNumber()) + 1);
                    }
                }
            }
        }
